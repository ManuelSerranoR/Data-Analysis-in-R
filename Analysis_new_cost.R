...
rotate <- function(vector, theta){
  rotationMatrix <- matrix(c(cos(theta), -sin(theta), sin(theta), cos(theta)), nrow = 2, ncol = 2)
  vector <- rotationMatrix %*% vector #Matrix multiplication
  return(vector)
}
...
target.pos <- as.matrix(data.ideal.path[,c(2,4)]) #Set a matrix with columns x2,y2, which are the end coordinates
...
for (i in trials[1:(length(trials))]){
  ...
  M <- weight_x*(dist2[,1])^2 + weight_y*(dist2[,2])^2 #Cost function
  Measure[aux] <- sum(M)
  aux = aux + 1
}
